...
function findjobs(position, location) {
  let result = [];
  let filtered = jobs.filter(
    (job) =>
      job.title.toUpperCase().includes(position.toUpperCase()) &&
  ...
  return result;
  //or
  //console.log(jobs.filter(job => job.title.toUpperCase().includes(position.toUpperCase()) && job.location.toUpperCase().includes(location.toUpperCase())).map(({description,requirements,benefits,company_profile,...j})=> {return j;}))
}
let results = findjobs("job", "US");
